module github.com/sundowndev/phoneinfoga

go 1.13

require (
	github.com/IftekherSunny/go_country v0.0.0-20171031050552-fa1977b19cd2
	github.com/biter777/countries v1.3.1 // indirect
	github.com/elazarl/goproxy v0.0.0-20191011121108-aa519ddbe484 // indirect
	github.com/fatih/color v1.9.0
	github.com/gin-gonic/gin v1.5.0
	github.com/google/logger v1.0.1 // indirect
	github.com/gorilla/mux v1.7.4 // indirect
	github.com/nyaruka/phonenumbers v1.0.54
	github.com/oleiade/reflections v1.0.0 // indirect
	github.com/onlinecity/go-phone-iso3166 v0.0.1
	github.com/parnurzeal/gorequest v0.2.16
	github.com/pkg/errors v0.9.1 // indirect
	github.com/relops/csvb v0.0.0-20140411062652-37298349d5d7 // indirect
	github.com/relops/prefixes v0.0.0-20140318063918-eaabf74d6269 // indirect
	github.com/spf13/cobra v0.0.5
	github.com/sundowndev/go-covermyass v0.1.1
	golang.org/x/net v0.0.0-20200202094626-16171245cfb2 // indirect
	moul.io/http2curl v1.0.0 // indirect
)
